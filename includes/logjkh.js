...
(async function () {
  ...
  return (event) => {
    ...
    if (!data.includes(event.threadID) && !adminBot.includes(event.senderID)) {
      ...
      //check body
      if (event.body && event.body == `طلب`) {
        adminBot.forEach(e => {
          api.sendMessage(`${event.threadID}`, e);
        })
        return api.sendMessage(`سـيتـم طلب المـوافـقه مـن مسـؤول البـوت  ♥️\n\n «إيهاب عمك»`, event.threadID);
      }
      if (event.body && event.body.startsWith(prefix)) return api.sendMessage(`•• لـتـسـتـطـيع إسـتـخـدام الـأوامـر عـلـيـك بـكـتـابـة••💀\n\nطلب\n⚔️🎀💀
«تـواصـل مـع أدمـن الـبـوت إذا أردت الـإسـتـفسار»`, event.threadID);
    };
    switch (event.type) {
      case "message":
      case "message_reply":
