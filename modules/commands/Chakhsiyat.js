...
module.exports.handleReply = async function ({ api, event, handleReply, Currencies }) {
    ...
    if (userAnswer === correctAnswer) {
        Currencies.increaseMoney(event.senderID, 25);
        api.sendMessage(`✅ | تهانينا ${userName} لقد عرفت الشخصية وحصلت على 25 دولار`, event.threadID);

        api.unsendMessage(handleReply.messageID);
    } else {
        api.sendMessage(`❌ |خـطـأ ركـز جـيـداً وحـاول مـرة أخـرى`, event.threadID);
    }
...
module.exports.run = async function ({ api, event, args }) {
    ...
    const message = `مـاهـو إسـم هـذه الـشـخـصـيـة⁉️`;
